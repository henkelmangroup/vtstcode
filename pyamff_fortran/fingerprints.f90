...
!f2py   INTENT(OUT) :: coeh
  ...
  40    REWIND 11
    ...
    SUBROUTINE read_mlff(nelement, MAX_FPS, filename)
  ...
  40    REWIND 11
        ...
        DO i = 1, nelements
            ...
            IF (ios == 59) THEN 
                CALL initialize_NN_params(uniq_elements) 
            ELSE
...
!f2py   INTENT(OUT) :: num_neigh
    ...
    SUBROUTINE load_default_mlff(nelement, MAX_FPS, uniqElems)
    !Ref: AMP  
        !Inputs
        INTEGER, INTENT(IN) :: nelement, MAX_FPS
        ...
        DO i=1, nelement
            ...
            !make sure cohesive energy is not used
            use_cohesive_energy=.FALSE.
        END DO  
        ...
        DO i=1, nelement
            DO j=1, nelement
                ...
                fpParas(i)%g1s(j)%r_cuts(1:nFPs)=6.0 !TODO: default?
        ...
        DO i=1, nelement
            ...
            DO j=1, nelement
                DO k=1, nelement
                    ...
                    fpParas(i)%g2s(j,k)%r_cuts = 6.0 !TODO: default?
                    fpParas(i)%g2s(j,k)%lambdas = 1.0 !TODO: default?
                    fpParas(i)%g2s(j,k)%theta_ss = 0.0 !TODO: default?
                    fpParas(i)%g2s(j,k)%gammas(1:2) = 1.0
                    fpParas(i)%g2s(j,k)%gammas(3:4) = -1.0 
    ...
    SUBROUTINE initialize_NN_params(uniqElems)
